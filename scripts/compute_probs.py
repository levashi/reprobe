...
if __name__ == "__main__":
    ...
    start_layer = config.get("model", {}).get("layer", 12)
    # 1. Chargement du dataset généré par ta v0
    data = torch.load("outputs/v1/acts_v001.pt")
    acts = data["acts"].float()    # Shape: [N, num_layers, hidden_dim]
    labels = data["labels"].float() # Shape: [N] (Scores Detoxify)
    ...
    probe_trainer.save("outputs/probes-v1")